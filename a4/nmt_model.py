...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = pad_packed_sequence(encoded[0], batch_first=True)
        init_decoder_hidden = self.h_projection(torch.reshape(encoded[1][0].permute(1,0,2), (encoded[1][0].shape[1], -1)))
        init_decoder_cell = self.c_projection(torch.cat((encoded[1][1][0], encoded[1][1][1]), 1))
