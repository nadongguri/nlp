...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            new_dependency = self.stack[-2:]
            self.stack.pop(-2)
            self.dependencies.append((new_dependency[-1], new_dependency[0]))
        elif transition == 'RA':
            new_dependency = self.stack[-2:]
            self.stack.pop(-1)
            self.dependencies.append((new_dependency[0], new_dependency[-1]))
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses != []:
        if len(unfinished_parses) < batch_size:
            minibatch = [unfinished_parses[0]]
        else:
            minibatch = unfinished_parses[:batch_size]
        next_transition = model.predict(minibatch)
        for m, next_transition in zip(minibatch, next_transition):
            m.parse_step(next_transition)
            if len(m.buffer) == 0 and len(m.stack) == 1:
                dependencies.extend([m.dependencies])
