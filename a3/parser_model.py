...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(self.embed_size * self.n_features, hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(
            nn.init.uniform_(torch.empty((hidden_size))))
        self.dropout = nn.Dropout(p = self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty((hidden_size, n_classes))))
        self.hidden_to_logits_bias = nn.Parameter(
            nn.init.uniform_(torch.empty((n_classes))))
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.view(w.shape[0] * w.shape[1]))
    ...
    def forward(self, w):
        ...
        x = F.relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h = F.relu(x)
        h = self.dropout(h)
